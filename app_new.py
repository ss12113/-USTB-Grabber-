...
class App(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("USTB 抢课神器 (终极自定义版)")
    ...
    def setup_ui(self):
        ...
        self.text_json.setPlaceholderText("在此粘贴 queryKxrw 的 Response JSON，解析后会自动保存...")
        ...
        control_group = QGroupBox("4. 参数配置与发射")
        ...
        # === 👇 核心修改：完全开放的时间配置 👇 ===
        # 学年：使用输入框，用户想填哪年填哪年
        self.edit_xn = QLineEdit("2025-2026")
        self.edit_xn.setPlaceholderText("例如: 2026-2027")
        self.edit_xn.setToolTip("请手动输入学年，例如 2025-2026")
        
        # 学期：下拉选择 1, 2, 3
        self.combo_xq = QComboBox()
        self.combo_xq.addItems(["1", "2", "3"])
        self.combo_xq.setCurrentText("2") # 默认选中2
        
        # 布局美化
        xn_xq_layout = QHBoxLayout()
        xn_xq_layout.addWidget(self.edit_xn)
        xn_xq_layout.addWidget(QLabel("学期:"))
        xn_xq_layout.addWidget(self.combo_xq)
        # ========================================
        ...
        control_layout.addRow("学年:", xn_xq_layout) # 插入学年配置行
        control_layout.addRow("类型:", self.combo_type)
    ...
    def toggle_grabbing(self, checked):
        if checked:
            ...
            # 获取用户实时设置
            xn = self.edit_xn.text().strip()
            xq = self.combo_xq.currentText()
            mode = self.combo_type.currentText()
            
            self.log(f"⚙️ 当前配置: {xn} 学期{xq} | {mode}")
    ...
    def send_request(self):
        ...
        # 🟢 2. 获取动态配置的时间参数（用户填什么就是什么）
        current_xn = self.edit_xn.text().strip()  # 例如 2026-2027
        current_xq = self.combo_xq.currentText()  # 例如 1
        current_xnxq = f"{current_xn}{current_xq}" # 自动拼成 2026-20271
        ...
        data = {
            ...
            'p_dqxn': current_xn, 
            'p_dqxq': current_xq, 
            'p_dqxnxq': current_xnxq,
            'p_xkfsdm': current_xkfsdm, # 动态课程类型
            # 👆 结束 👆
